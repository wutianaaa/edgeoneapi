...
async function syncChannelModelMappings(kv, input) {
  ...
  const selectedModelIds = new Set(
    (Array.isArray(input.model_ids) ? input.model_ids : [])
  ...
  for (const existing of existingModels) {
    ...
    const shouldHaveChannel = selectedModelIds.has(existing.model);
    ...
    if (shouldHaveChannel) {
      ...
      nextMappings = {
        ...currentMappings,
        [channelId]: existing.model
      };
      selectedModelIds.delete(existing.model);
    } else if (hadChannel) {
  ...
  for (const modelId of selectedModelIds) {
    const model = normalizeModel({
      ...
      channel_mappings: { [channelId]: modelId }
    });
    await putJson(kv, modelKey(model.model), model);
  }
